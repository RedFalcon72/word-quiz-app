...
const Quiz = () => {
  ...
  // シャッフルせず、元の配列（book.cards）をそのまま使用する
  ...
  return (
    <div className="max-w-lg mx-auto p-6 space-y-8">
      ...
      <div
        ...
        className={`relative h-80 w-full cursor-pointer transition-all duration-500 [perspective:1000px] ${isFlipped ? "[transform:rotateY(180deg)]" : ""}`}
      >
        <div className="absolute inset-0 w-full h-full [transform-style:preserve-3d]">
          <div className="absolute inset-0 w-full h-full bg-white border-4 border-slate-900 rounded-[2.5rem] flex items-center justify-center p-8 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] [backface-visibility:hidden]">
      ...
      <div
        className={`grid grid-cols-2 gap-4 transition-all duration-300 ${isFlipped ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}
      >
        <button
          onClick={(e) => {
            e.stopPropagation();
